...
@pytest.mark.parametrize("event_type", ["assistant.message", SimpleNamespace(value="assistant.message")])
def test_event_type_value_returns_string(event_type: str | SimpleNamespace) -> None:
    event = SimpleNamespace(type=event_type)

    assert GitHubCopilotTarget._event_type_value(event=event) == "assistant.message"
